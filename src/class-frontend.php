...
class Frontend {
	...
	public function gp_pre_tmpl_load( $template, $args ) {
		...
		$options = array(
			...
			'nonce'   => wp_create_nonce( 'gpdeepseek_nonce' ),
		);

		wp_register_script( 'gp-translate-with-deepseek-js', plugins_url( 'assets/gpdeepseek_translate.js', __DIR__ ), array( 'jquery', 'editor', 'gp-common' ), '1.0', true );
		gp_enqueue_script( 'gp-translate-with-deepseek-js' );
		wp_localize_script( 'gp-translate-with-deepseek-js', 'gpdeepseek_translate', $options );
	}
	...
	public function gp_entry_actions( array $actions ): array {
		...
		$actions[] = '<a href="#" class="gpdeepseek_translate" tabindex="-1">' . esc_html__( 'Translate with DeepSeek', 'gp-translate-with-deepseek' ) . '</a><br>';
	...
	public function gp_translation_set_bulk_action(): void {
		...
		echo '<option value="gpdeepseek_translate">' . esc_html__( 'Translate with DeepSeek', 'gp-translate-with-deepseek' ) . '</option>';
	}
	...
	public function gp_translation_set_bulk_action_post( $project, $locale, $translation_set, $bulk ): void {
		...
		if ( 'gpdeepseek_translate' !== $bulk['action'] ) {
			return;
		}
